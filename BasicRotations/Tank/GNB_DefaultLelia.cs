...
[Api(3)]
public sealed class GNB_DefaultLelia : GunbreakerRotation
{
    ...
    protected override bool EmergencyAbility(IAction nextGCD, out IAction? act)
    {
        ...
        if (InCombat && !Player.HasStatus(true, StatusID.NoMercy) && HeartOfLightPvE.CanUse(out act, skipAoeCheck: true)) return true;
        if (InCombat && HostileTarget.DistanceToPlayer() <= 5 && !Player.HasStatus(true, StatusID.NoMercy) && ReprisalPvE.CanUse(out act, skipAoeCheck: true)) return true;

        //10
        if (CamouflagePvE.CanUse(out act)) return true;
        //15
        if (InCombat && HeartOfStonePvE.CanUse(out act)) return true;
        ...
        if (InCombat && HostileTarget.DistanceToPlayer() <= 5 && ReprisalPvE.CanUse(out act)) return true;
