...
public sealed class SMN_LeliaDefaultPvP : SummonerRotation
{
    ...
    protected override bool GeneralGCD(out IAction? act)
    {
        ...
        if (LimitBreakLevel >=1 && !HostileTarget.HasStatus(true, StatusID.Guard) && LBInPvP && HostileTarget &&
                Target.GetHealthRatio()* 100 <= SBValue && Player.GetHealthRatio()* 100 >= SPValue)
            {
                if (!HostileTarget.HasStatus(true, StatusID.Guard) && SummonBahamutPvP.CanUse(out act, usedUp: true, skipAoeCheck: true)) return true;
            }
            else if (LimitBreakLevel>=1 && !Target.HasStatus(true, StatusID.Guard) && LBInPvP && HostileTarget &&
                Target.GetHealthRatio()*100 <= SPValue && Player.GetHealthRatio()* 100 < SPValue)
            {
                if (!HostileTarget.HasStatus(true, StatusID.Guard) && SummonPhoenixPvP.CanUse(out act, usedUp: true, skipAoeCheck: true)) return true;
            }

            //if (CrimsonStrikePvP.CanUse(out act, usedUp: true, skipAoeCheck: true)) return true;
            if (CCPvP && HostileTarget && Target.GetHealthRatio()*100 < CrimsonValue)
            {
        ...
        if (!HostileTarget.HasStatus(true, StatusID.Guard) && 
            ...
            if (!HostileTarget.HasStatus(true, StatusID.Guard) && Player.HasStatus(true, StatusID.DreadwyrmTrance_3228) &&
                AstralImpulsePvP.CanUse(out act, usedUp: true, skipAoeCheck: true)) return true;
            if (!HostileTarget.HasStatus(true, StatusID.Guard) && Player.HasStatus(true, StatusID.FirebirdTrance) &&
    ...
    protected override bool AttackAbility(out IAction? act)
    {
        if (!HostileTarget.HasStatus(true, StatusID.Guard))
        {
            ...
            if (!HostileTarget.HasStatus(true, StatusID.Guard) && InCombat &&
